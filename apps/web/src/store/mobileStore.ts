...
interface MobileState {
  ...
  // Offline Drafts (generic KV store for form fields)
  ...
  enqueuePhoto: (item: { openingId: string; appointmentId: string; file?: string; localUrl?: string }) => string;
  ...
  persist(
    (set, get) => ({
      ...
      enqueuePhoto: (item) => {
        ...
        set(s => ({
          photoQueue: [...s.photoQueue, { ...item, id, status: 'pending', retryCount: 0, createdAt: Date.now() }]
        }));
        return id;
      },
