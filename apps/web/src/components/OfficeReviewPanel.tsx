    ...
    <div className="office-panel">
      ...
      {tab === 'approval' && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {/* Validation summary */}
            <div style={{ padding: '0.5rem', background: 'var(--bg-card)', borderRadius: 'var(--radius-sm)', border: `1px solid ${validation.blockers > 0 ? '#ef4444' : '#22c55e'}` }}>
              <div style={{ fontSize: '0.625rem', fontWeight: 700, color: 'var(--text-muted)', marginBottom: '0.25rem' }}>VALIDATION</div>
              <div style={{ display: 'flex', gap: '0.75rem', fontSize: '0.75rem' }}>
                <span style={{ color: validation.blockers > 0 ? '#ef4444' : '#22c55e' }}>🛑 {validation.blockers} Blockers</span>
                <span style={{ color: validation.high > 0 ? '#f59e0b' : '#22c55e' }}>⚠ {validation.high} High</span>
            ...
            <div style={{ padding: '0.5rem', background: 'var(--bg-card)', borderRadius: 'var(--radius-sm)', border: `1px solid ${data.approval.officeApproved ? '#22c55e' : 'var(--border)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {!data.approval.officeApproved
                  ? <button onClick={approveOffice} className="btn btn-sm" disabled={validation.blockers > 0} style={{ fontSize: '0.625rem', background: '#22c55e', color: 'white', border: 'none', opacity: validation.blockers > 0 ? 0.5 : 1 }}>Approve</button>
      ...
      {tab === 'packet' && (
        <div>
          ...
          <button onClick={() => window.print()} className="btn btn-primary" style={{ width: '100%', marginTop: '0.75rem', fontSize: '0.8125rem' }}>
            🖨 Print Processing Packet
          </button>
          <div style={{ fontSize: '0.5625rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '0.25rem' }}>
            Tip: Use browser Print → Save as PDF
          </div>
        </div>
      )}
    </div>
