...
export function SignaturePad({
  ...
  return (
    <div style={{ position: 'relative' }}>
      {/* Signature canvas */}
      <div style={{
        border: `2px solid ${saved ? '#22c55e' : isEmpty ? 'rgba(59,130,246,0.4)' : '#3b82f6'}`,
        borderRadius: 8, overflow: 'hidden', background: '#fff',
        position: 'relative', cursor: 'crosshair',
        boxShadow: saved ? '0 0 0 3px rgba(34,197,94,0.2)' : '0 0 0 3px rgba(59,130,246,0.1)',
        transition: 'all 0.2s',
      }}>
        {isEmpty && (
          <div style={{
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
        <button onClick={handleClear} style={{ padding: '0.5rem 1rem', fontSize: '0.8125rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, color: '#94a3b8', cursor: 'pointer' }}>
          Clear
        </button>
        <button onClick={handleSave} disabled={isEmpty}
          style={{ flex: 1, padding: '0.5rem 1rem', fontSize: '0.875rem', fontWeight: 700, background: isEmpty ? 'rgba(59,130,246,0.3)' : '#3b82f6', border: 'none', borderRadius: 6, color: 'white', cursor: isEmpty ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
          {saved ? '✓ Saved' : `Save ${isInitials ? 'Initials' : 'Signature'}`}
        </button>
      </div>
      {saved && (
        <div style={{ textAlign: 'center', fontSize: '0.75rem', color: '#22c55e', marginTop: '0.25rem', fontWeight: 600 }}>
