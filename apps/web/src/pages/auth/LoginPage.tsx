...
export function LoginPage() {
  ...
  const googleLogin = useGoogleLogin({
    // Use token response (access_token) — avoids redirect flow entirely.
    // The popup opens, user picks account, token comes back via postMessage.
    flow: 'implicit',
    onSuccess: async (tokenResponse: any) => {
      setLoading(true);
      try {
        // Send the access_token to our backend which verifies it with Google
    ...
    onError: (error) => {
      ...
      toast.error('Google Sign-In failed. Please use email login below.');
    },
  });
