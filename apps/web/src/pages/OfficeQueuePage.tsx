...
function getOfficeStatus(appt: any): string {
  const v = validateAppointment(appt);
  const coach = runAppointmentCoach(appt);
  if (v.blockers > 0 || coach.overallRisk === 'HIGH_RISK') return 'missing_info';
  if (coach.openingScores.some(o => o.measurementConfidence < 50)) return 'needs_remeasure';
  if (v.high > 0 || coach.overallRisk === 'REVIEW') return 'needs_review';
  if (v.readyState === 'ready_to_export') return 'ready_production';
