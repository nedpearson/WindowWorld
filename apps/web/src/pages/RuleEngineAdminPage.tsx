...
export function RuleEngineAdminPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Fallback to seed data if API not yet seeded
      setRules([
        { id: 'rule_brick_ext', name: 'Brick → EXT Install', description: 'Brick exterior defaults Install Type to EXT', triggerField: 'exteriorType', triggerValue: 'Brick', actionType: 'set_field', actionField: 'installType', actionValue: 'EXT', isActive: true },
        { id: 'rule_siding_int', name: 'Siding → INT Install', description: 'Siding exterior defaults to INT Install and requires trim', triggerField: 'exteriorType', triggerValue: 'Siding', actionType: 'require_confirmation', actionField: 'installType', actionValue: 'INT', isActive: true },
        { id: 'rule_tempered_warning', name: 'Tempered Glass Warning', description: 'Warn if bathroom window lacks tempered glass', triggerField: 'roomLocation', triggerValue: 'Bath', actionType: 'warn', message: 'Bathroom window may require tempered glass', isActive: true },
      ]);
    }
    setLoading(false);
  };
