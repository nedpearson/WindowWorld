...
// Simulates: senior manager, remeasure tech, installer,
// office processor, and QC reviewer simultaneously.
// ═══════════════════════════════════════════════════════════
...
function runContextualChecks(op: any, houseMap: any, appointment: any): CoachItem[] {
  ...
  if ((op.floorNumber || 1) >= 2) {
    if (!op.installNotes?.toLowerCase()?.includes('ladder') && !op.installNotes?.toLowerCase()?.includes('scaffold'))
      items.push({ id: `ctx-ladder-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: 2nd floor — add ladder/scaffold note`, detail: 'Installers need to know access requirements for upper floors', jumpStep: 3 });
    if (!op.installNotes?.toLowerCase()?.includes('access'))
      items.push({ id: `ctx-access-${n}`, category: 'installer', severity: 'info', openingNumber: n, message: `Opening #${n}: Add access/difficulty note for floor ${op.floorNumber}`, jumpStep: 3 });
  }

  // Specialty shape
  if (SPECIALTY.includes(op.productCategory)) {
    if (!op.radius && !op.customRadius)
      items.push({ id: `ctx-radius-${n}`, category: 'measurement', severity: 'critical', openingNumber: n, message: `Opening #${n}: Specialty shape requires radius`, detail: 'Production cannot order without radius measurement', jumpStep: 3 });
    if (!op.legHeight)
      items.push({ id: `ctx-leg-${n}`, category: 'measurement', severity: 'warning', openingNumber: n, message: `Opening #${n}: Specialty shape — verify leg height`, jumpStep: 3 });
    items.push({ id: `ctx-spec-verify-${n}`, category: 'measurement', severity: 'info', openingNumber: n, message: `Opening #${n}: Specialty shape — production verification recommended`, jumpStep: 3 });
  }

  // Tempered glass likely situations
  if ((op.floorNumber || 1) === 1 && op.height && op.height < 24) {
    if (op.temperedGlass === 'none' || !op.temperedGlass)
      items.push({ id: `ctx-tempered-${n}`, category: 'forgotten', severity: 'warning', openingNumber: n, message: `Opening #${n}: Low window — confirm tempered glass not needed`, detail: 'Windows near floor level often require tempered glass per building code', jumpStep: 3 });
  }
  if (op.roomLocation?.toLowerCase()?.match(/bath|shower|tub/)) {
    if (op.temperedGlass === 'none' || !op.temperedGlass)
      items.push({ id: `ctx-bath-temp-${n}`, category: 'forgotten', severity: 'critical', openingNumber: n, message: `Opening #${n}: Bathroom — tempered glass likely required`, detail: 'Building code requires tempered glass within 60" of water source', jumpStep: 3 });
    if (op.obscureGlass === 'none' || !op.obscureGlass)
      items.push({ id: `ctx-bath-obsc-${n}`, category: 'forgotten', severity: 'warning', openingNumber: n, message: `Opening #${n}: Bathroom — consider obscure glass`, jumpStep: 3 });
  }
  ...
  if (op.exteriorType?.toLowerCase()?.includes('brick') || appointment?.houseMap?.exteriorMaterial?.toLowerCase()?.includes('brick')) {
    if (!op.installNotes?.toLowerCase()?.includes('brick'))
      items.push({ id: `ctx-brick-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: Brick exterior — add brickmold/install detail`, detail: 'Installer needs to know return depth and brickmold condition', jumpStep: 3 });
  }
  ...
  if (op.sillRepair || op.installNotes?.toLowerCase()?.includes('sill') || op.installNotes?.toLowerCase()?.includes('rot')) {
    if (!op.installerNotes?.toLowerCase()?.includes('sill'))
      items.push({ id: `ctx-sill-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: Sill repair flagged — add detailed repair note`, jumpStep: 3 });
  }

  // Patio door
  if (DOORS.includes(op.productCategory)) {
    if (!op.hinge)
      items.push({ id: `ctx-door-hinge-${n}`, category: 'forgotten', severity: 'critical', openingNumber: n, message: `Opening #${n}: Door — hinge side not specified`, jumpStep: 3 });
    if (!op.installNotes?.toLowerCase()?.includes('track') && !op.installNotes?.toLowerCase()?.includes('threshold'))
      items.push({ id: `ctx-door-track-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: Door — add track/threshold notes`, jumpStep: 3 });
  }

  // Lead paint
  if (appointment?.customer?.preLead1978) {
    items.push({ id: `ctx-lead-${n}`, category: 'installer', severity: 'critical', openingNumber: n, message: `Opening #${n}: Pre-1978 home — lead containment required`, jumpStep: 3 });
  }

  // Missing room/elevation
  if (!op.roomLocation)
    items.push({ id: `ctx-room-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: No room label — installer won't know which window`, jumpStep: 3 });
  if (!op.elevation)
    items.push({ id: `ctx-elev-${n}`, category: 'installer', severity: 'warning', openingNumber: n, message: `Opening #${n}: No elevation — which side of house?`, jumpStep: 3 });
...
function checkConsistency(appointment: any): CoachItem[] {
  ...
  if (appointment.subtotal > 0 && Math.abs(computedSubtotal - appointment.subtotal) > 0.01) {
    items.push({ id: 'con-subtotal', category: 'contract', severity: 'critical', message: `Subtotal mismatch: openings=$${computedSubtotal.toFixed(2)} vs quote=$${appointment.subtotal.toFixed(2)}`, jumpStep: 4 });
  }

  // Check for openings with zero price
  const zeroPriced = openings.filter((o: any) => !o.totalPrice || o.totalPrice <= 0);
  if (zeroPriced.length > 0) {
    items.push({ id: 'con-zero-price', category: 'pricing', severity: 'critical', message: `${zeroPriced.length} opening(s) have no price — quote is incomplete`, jumpStep: 4 });
  }

  // Needs-verification check
  const unverified = openings.filter((o: any) => o.needsVerification);
  if (unverified.length > 0) {
    items.push({ id: 'con-unverified', category: 'pricing', severity: 'warning', message: `${unverified.length} opening(s) have unverified pricing`, jumpStep: 4 });
  }

  // Tax sanity
  if (appointment.totalAmount > 0 && appointment.taxAmount <= 0) {
    items.push({ id: 'con-tax', category: 'pricing', severity: 'warning', message: 'No tax calculated — verify tax rate', jumpStep: 4 });
  }
...
export function runAppointmentCoach(appointment: any): CoachResult {
  ...
  for (const op of openings) {
    ...
    if (photoReqs.length > 0 && existingPhotos === 0) {
      items.push({ id: `photo-req-${op.openingNumber}`, category: 'photo', severity: 'warning', openingNumber: op.openingNumber, message: `Opening #${op.openingNumber}: ${photoReqs.length} photo(s) recommended`, detail: photoReqs.join(', '), jumpStep: 3 });
    }

    // Low clarity warning
    if (clarity < 50) {
      items.push({ id: `clarity-low-${op.openingNumber}`, category: 'installer', severity: 'warning', openingNumber: op.openingNumber, message: `Opening #${op.openingNumber}: Installer clarity ${clarity}% — add more detail`, detail: 'An installer may not understand this opening. Add room, elevation, notes, and sketch marker.', jumpStep: 3 });
    }

    // Low confidence warning
    if (confidence < 50) {
      items.push({ id: `meas-low-${op.openingNumber}`, category: 'measurement', severity: 'warning', openingNumber: op.openingNumber, message: `Opening #${op.openingNumber}: Measurement confidence ${confidence}% — remeasure recommended`, jumpStep: 3 });
    }
  ...
  for (const item of commonlyForgotten) {
    items.push({ id: `forgot-${item.slice(0, 20)}`, category: 'forgotten', severity: 'tip', message: `Commonly forgotten: ${item}`, jumpStep: 3 });
  }
  ...
  const hasTax = appointment.taxAmount > 0 ? 1 : 0;
  const hasTotal = appointment.totalAmount > 0 ? 1 : 0;
  const hasSigs = (appointment.signatures?.length || 0) > 0 ? 1 : 0;
  const contractAccuracyScore = Math.round(((hasDeposit + hasTax + hasTotal + hasSigs) / 4) * 100);
