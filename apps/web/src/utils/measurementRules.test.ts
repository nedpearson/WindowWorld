...
describe('Specialty Dimension Sets', () => {
  ...
  test('Oriel requires topSashWidth and topSashHeight', () => {
    const spec = getSpecialtyDimensionSet('oriel');
    const keys = spec!.requiredDimensions.map(d => d.key);
    expect(keys).toContain('topSashWidth');
    expect(keys).toContain('topSashHeight');
  });
...
describe('Measurement Export Readiness', () => {
  ...
  test('Oriel WITH top sash confirmation is not blocked', () => {
    const openings = [{ openingNumber: 1, model: 'Oriel Window', productCategory: 'oriel', topSashConfirmed: true }];
    const result = checkMeasurementExportReadiness(openings, {});
    expect(result.blocked).toBe(false);
  });
  ...
  test('Approved adjustment with verified rule clears export', () => {
    const openings = [{ openingNumber: 1, productCategory: 'oriel', topSashConfirmed: true }];
    const adj: MeasurementAdjustment = {
