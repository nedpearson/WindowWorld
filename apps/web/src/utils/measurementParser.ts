...
export function parseMeasurement(input: string): ParsedMeasurement {
  ...
  // Try numeric fraction format: "35 3/8" or "35-3/8"
  const fracMatch = raw.match(/^(\d+)\s*[-\s]\s*(\d+)\/(\d+)$/);
  if (fracMatch) {
    const whole = parseInt(fracMatch[1]);
    const num = parseInt(fracMatch[2]);
    const den = parseInt(fracMatch[3]);
    if (den > 0) inches = whole + num / den;
    return formatResult(inches, warnings);
  }

  // Try bare fraction: "3/8"
  const bareFrac = raw.match(/^(\d+)\/(\d+)$/);
  if (bareFrac) {
    const num = parseInt(bareFrac[1]);
    const den = parseInt(bareFrac[2]);
  ...
  if (decMatch) {
    inches = parseFloat(decMatch[1]);
    return formatResult(inches, warnings);
  }

  // Try voice/text: "thirty five and three eighths"
  ...
  if (!isNaN(num)) return formatResult(num, warnings);
