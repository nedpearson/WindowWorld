...
router.get('/aging', auth.manager, async (req, res) => {
  const user = (req as AuthenticatedRequest).user;
...
router.post('/from-proposal', auth.repOrAbove, async (req, res) => {
  const user = (req as AuthenticatedRequest).user;
  const data = await invoicesService.createFromProposal({ ...req.body, createdById: user.id });
  res.status(201).json({ success: true, data });
});

router.post('/:id/payments', auth.repOrAbove, async (req, res) => {
  const user = (req as AuthenticatedRequest).user;
  const data = await invoicesService.recordPayment(req.params.id, { ...req.body, recordedById: user.id });
  res.status(201).json({ success: true, data });
});

router.post('/:id/send', auth.repOrAbove, async (req, res) => {
  const user = (req as AuthenticatedRequest).user;
  const data = await invoicesService.send(req.params.id, user.id);
  res.json({ success: true, data });
});
