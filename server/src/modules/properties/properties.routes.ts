...
router.get('/:id', auth.repOrAbove, async (req: Request, res: Response) => {
  const user = (req as AuthenticatedRequest).user;
  const property = await propertiesService.getById((req.params.id as string), user.organizationId);
...
router.get('/:id/order-readiness', auth.repOrAbove, async (req: Request, res: Response) => {
  const user = (req as AuthenticatedRequest).user;
  const readiness = await propertiesService.getOrderReadiness((req.params.id as string), user.organizationId);
...
router.post('/', auth.repOrAbove, async (req: Request, res: Response) => {
  const user = (req as AuthenticatedRequest).user;
  const property = await propertiesService.create({ ...req.body, organizationId: user.organizationId }, user.id);
...
router.patch('/:id', auth.repOrAbove, async (req: Request, res: Response) => {
  const user = (req as AuthenticatedRequest).user;
  const property = await propertiesService.update((req.params.id as string), user.organizationId, req.body, user.id);
  res.json({ success: true, data: property });
});
...
router.post('/:id/link-lead', auth.repOrAbove, async (req: Request, res: Response) => {
  const user = (req as AuthenticatedRequest).user;
  const property = await propertiesService.linkToLead((req.params.id as string), user.organizationId, req.body.leadId);
  res.json({ success: true, data: property });
});
