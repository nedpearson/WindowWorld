...
class OpenAIProvider {
    ...
    get client() {
        if (!this._client) {
            ...
            this._client = new openai_1.default({ apiKey: process.env.OPENAI_API_KEY });
        }
        return this._client;
    }
...
function getProvider() {
    if (!process.env.OPENAI_API_KEY) {
        logger_1.logger.warn('OPENAI_API_KEY not set - AI features disabled. Add it in Railway environment variables.');
    ...
    switch (provider) {
        ...
        default:
            logger_1.logger.warn(`Unknown AI provider "${provider}" â€” falling back to OpenAI`);
            return new OpenAIProvider();
    }
}
...
Return a JSON object with this exact structure:
{
  ...
  "measurementDisclaimer": "AI-ESTIMATED â€” REQUIRES HUMAN VERIFICATION BEFORE ORDERING"
        ...
        try {
            ...
            logger_1.logger.error('Window photo analysis failed:', error);
            await prisma_1.prisma.aiAnalysis.create({
                data: {
                    leadId: params.leadId,
                    openingId: params.openingId,
                    analysisType: 'window-classification',
...
Return JSON with this structure:
{
  ...
  "rationale": "Why these recommendations were made"
        ...
        try {
            ...
            logger_1.logger.error('Lead pitch generation failed:', error);
...
Return JSON with this exact structure:
{
  ...
  "financingAngle": "If applicable, how to introduce financing to this lead"
    ...
    async generateLeadSummary(lead) {
        const recentActivities = (lead.activities || []).slice(0, 5).map((a) => `${a.type}: ${a.title}`).join('; ');
        const appointments = (lead.appointments || []).map((a) => `${a.type} on ${new Date(a.scheduledAt).toLocaleDateString()}`).join(', ');
