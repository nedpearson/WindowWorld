...
exports.campaignsService = exports.CampaignsService = exports.SMS_TEMPLATES = exports.EMAIL_TEMPLATES = exports.CAMPAIGN_TEMPLATES = void 0;
...
function buildEmailHtml(data, body) {
...
class CampaignsService {
    ...
    async templates() {
        return Object.entries(exports.CAMPAIGN_TEMPLATES).map(([key, t]) => ({
            id: key,
            ...t,
            stepCount: t.steps.length,
        }));
    }
    async enroll(leadId, campaignTemplateKey, enrolledById) {
        const template = exports.CAMPAIGN_TEMPLATES[campaignTemplateKey];
        if (!template)
            throw new Error(`Campaign template not found: ${campaignTemplateKey}`);
        const lead = await prisma_1.prisma.lead.findUnique({ where: { id: leadId }, include: { contacts: { where: { isPrimary: true } } } });
        if (!lead)
            throw new Error(`Lead not found: ${leadId}`);
        // Check not already enrolled
        const existing = await prisma_1.prisma.campaignEnrollment?.findFirst({
            where: { leadId, campaign: { templateKey: campaignTemplateKey }, status: 'ACTIVE' },
        });
        if (existing)
            return { enrolled: false, message: 'Lead already enrolled in this campaign' };
        // Find or create campaign record
        const lead_with_org = await prisma_1.prisma.lead.findUnique({ where: { id: leadId }, select: { organizationId: true } });
        let campaign = await prisma_1.prisma.campaign.findFirst({
            where: { templateKey: campaignTemplateKey, organizationId: lead_with_org.organizationId },
        });
        if (!campaign) {
            campaign = await prisma_1.prisma.campaign.create({
                data: {
                    name: template.name,
        ...
        const enrollment = await prisma_1.prisma.campaignEnrollment?.create({
            data: {
    ...
    async executeStep(params) {
        ...
        // Find enrollment
        const enrollment = await prisma_1.prisma.campaignEnrollment?.findFirst({
            where: { leadId, status: 'ACTIVE' },
            include: { campaign: true },
        });
        if (!enrollment) {
            logger_1.logger.warn(`No active enrollment for lead ${leadId}`);
            return;
        }
        const templateKey = enrollment?.campaign?.templateKey;
        const template = exports.CAMPAIGN_TEMPLATES[templateKey];
        if (!template)
            return;
        ...
        const lead = await prisma_1.prisma.lead.findUnique({
            where: { id: leadId },
            include: { contacts: { where: { isPrimary: true } }, assignedRep: true },
        });
        if (!lead)
            return;
        const repName = lead.assignedRep ? `${lead.assignedRep.firstName} ${lead.assignedRep.lastName}` : 'Your WindowWorld Rep';
        ...
        if (stepConfig.type === 'EMAIL') {
            ...
            const emailTemplate = exports.EMAIL_TEMPLATES[emailTemplateKey];
            if (emailTemplate) {
                const rendered = emailTemplate(templateData);
                const toEmail = primaryContact?.email || lead.email;
                if (toEmail) {
                    await jobs_1.emailQueue.add('send-campaign-email', {
                        to: toEmail,
        ...
        if (stepConfig.type === 'SMS') {
            const smsTemplate = exports.SMS_TEMPLATES[stepConfig.templateKey];
            if (smsTemplate) {
                const body = smsTemplate(templateData);
                // SMS dispatch via Twilio wrapper
                const phone = primaryContact?.phone || lead.phone;
                if (phone) {
                    await sms_service_1.smsService.sendSms(phone, body);
                }
                else {
                    logger_1.logger.warn(`Could not send SMS for lead ${leadId} - no phone number`);
                }
            }
        }
        // Log activity
        ...
        if (nextStepConfig) {
            await prisma_1.prisma.campaignEnrollment?.update({
            ...
            logger_1.logger.info(`Campaign complete for lead ${leadId}: ${templateKey}`);
        }
    }
    async scheduleNextStep(leadId, templateKey, step, lead) {
        const template = exports.CAMPAIGN_TEMPLATES[templateKey];
        const stepConfig = template?.steps.find((s) => s.step === step);
        if (!stepConfig)
            return;
        ...
        const { automationQueue: q } = await Promise.resolve().then(() => __importStar(require('../../jobs')));
    ...
    async unenroll(leadId, reason) {
        await prisma_1.prisma.campaignEnrollment?.updateMany({
            where: { leadId, status: 'ACTIVE' },
            data: { status: 'UNSUBSCRIBED', unenrolledAt: new Date(), unenrollReason: reason },
        });
    }
    async triggerForStatus(leadId, status, enrolledById) {
        const matching = Object.entries(exports.CAMPAIGN_TEMPLATES)
