...
class PdfGeneratorService {
    outputDir;
    constructor() {
        this.outputDir = process.env.PDF_OUTPUT_DIR || path_1.default.join(process.cwd(), 'pdfs');
        if (!fs_1.default.existsSync(this.outputDir)) {
            fs_1.default.mkdirSync(this.outputDir, { recursive: true });
        }
    }
    async generate(proposal) {
        ...
        try {
            // Try puppeteer first
            const puppeteer = await Promise.resolve().then(() => __importStar(require('puppeteer'))).catch(() => null);
            if (puppeteer) {
                ...
                await browser.close();
                logger_1.logger.info(`PDF generated via Puppeteer: ${outputPath}`);
                return outputPath;
            }
        }
        catch (err) {
            logger_1.logger.warn(`Puppeteer unavailable: ${err.message} â€” falling back to HTML file`);
        }
        // Fallback: save as HTML
        const htmlPath = outputPath.replace('.pdf', '.html');
        fs_1.default.writeFileSync(htmlPath, html);
        logger_1.logger.info(`Proposal HTML written (PDF unavailable): ${htmlPath}`);
        return htmlPath;
    }
    buildHtml(proposal) {
...
<div class="content">
  ...
  <section>
    ...
    ${lineItems.some((i) => i.isAiEstimated) ? `
  ...
  <section>
    ...
    <ul class="warranty-list">
      ${(Array.isArray(proposal.warrantyHighlights) ? proposal.warrantyHighlights : defaultWarranty).map((w) => `<li>${w}</li>`).join('')}
    </ul>
  </section>
