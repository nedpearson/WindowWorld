...
exports.productsService = exports.ProductsService = exports.FINANCING_OPTIONS = exports.OPTION_PRICES = exports.WINDOW_SERIES = void 0;
const errorHandler_1 = require("../../shared/middleware/errorHandler");
// â”€â”€â”€ Louisiana window series pricing (realistic Window World pricing) â”€â”€
exports.WINDOW_SERIES = {
...
class ProductsService {
    catalog() {
        return Object.entries(exports.WINDOW_SERIES).map(([id, product]) => ({
            id,
            ...product,
            options: Object.entries(exports.OPTION_PRICES)
                .filter(([, opt]) => opt.included.includes(id))
                .map(([optId, opt]) => ({ id: optId, name: opt.name, price: opt.price, included: true })),
        }));
    }
    getById(id) {
        const product = exports.WINDOW_SERIES[id];
        if (!product)
            throw new errorHandler_1.NotFoundError('Product');
        return { id, ...product, allOptions: exports.OPTION_PRICES };
    }
    /**
    ...
    calculateWindowPrice(params) {
        ...
        const series = exports.WINDOW_SERIES[seriesId];
        if (!series)
            throw new errorHandler_1.NotFoundError('Product series');
        ...
        for (const optId of options) {
            const opt = exports.OPTION_PRICES[optId];
        ...
        const installOption = exports.OPTION_PRICES['INSTALL_STANDARD'];
    ...
    calculateMonthlyPayment(principal, finOptionId) {
        const option = exports.FINANCING_OPTIONS.find((f) => f.id === finOptionId);
        if (!option)
            throw new errorHandler_1.NotFoundError('Financing option');
        if (option.aprPct === 0) {
            return {
    ...
    financingOptions() {
        return exports.FINANCING_OPTIONS;
    }
}
