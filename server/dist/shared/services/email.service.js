...
async function sendEmail(opts) {
    ...
    if (!resend) {
        ...
        logger_1.logger.warn('[email] RESEND_API_KEY not set — email not sent (mock)', {
            to: opts.to,
    ...
    try {
        ...
        if (error) {
            logger_1.logger.error('[email] Resend API error', { error, to: opts.to, subject: opts.subject });
            return { provider: 'resend', success: false, error: error.message };
        }
        logger_1.logger.info(`[email] Sent via Resend: id=${data?.id} to=${Array.isArray(opts.to) ? opts.to.join(',') : opts.to}`);
        return { provider: 'resend', success: true, id: data?.id };
    }
    catch (err) {
        logger_1.logger.error('[email] Resend send exception', { message: err.message, to: opts.to });
...
async function sendProposalEmail(opts) {
    const { to, customerName, repName, repPhone, repEmail, proposalTitle, proposalId, windowCount, totalAmount, expiresAt, pdfUrl, appBaseUrl, } = opts;
